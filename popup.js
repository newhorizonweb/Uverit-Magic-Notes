...
// Popup Width
const breakWidth = 552;
let popupWidth = breakWidth;

// Port
const port = chrome.runtime.connect();
port.postMessage({ type: 'currentPageWidth' });

port.onMessage.addListener((message) => {
    if (message.type === 'pageWidth'){

        // When the extension popup is opened on a page without a URL
        // Like a Speed Dial, it makes the page width = 0
        // Set the default popup width when that's the case
        const pageWidth = message.width ? message.width : breakWidth;

        // Make the popup smaller on mobile devices
        // Min width is set in css
        if (pageWidth < breakWidth){
            popupWidth = pageWidth;
        } else {
            popupWidth = breakWidth;
        }
